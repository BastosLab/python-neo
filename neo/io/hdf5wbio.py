...
class Hdf5WbIO(BaseIO):
    ...
    def read_all_blocks(self, lazy=False, **kwargs):
        ...
        try:
            self._file = h5py.File(self.filename, mode=self.nwb_file_mode)
        except ValueError:
    ...
    def _read_timeseries_group(self, group_name, lazy):
        ...
        for timeseries in group.values():
            ...
            if isinstance(timeseries, pynwb.misc.AnnotationSeries):
            ...
            elif 'rate' in timeseries or _regularly_sampled(timeseries):  # AnalogSignal
                ...
                signal = IrregularlySampledSignal(timestamps, signal,
                                                  name=group_name,
                                                  units="seconds")
                if not lazy:
                    signal = signal.load()
...
class AnalogSignalProxy(BaseAnalogSignalProxy):
    ...
    def __init__(self, timeseries, nwb_group):
        ...
        self.units = timeseries.get('unit', None)
        if timeseries.get('conversion', None):
            self.units = _recompose_unit(timeseries.unit, timeseries.conversion)
        if timeseries.get('starting_time, None') is not None:
            self.t_start = timeseries['starting_time'] * pq.s
        else:
            self.t_start = timeseries['timestamps'][0] * pq.s
        self.sampling_rate = timeseries['rate'] * pq.Hz if 'rate' in timeseries\
                             else None
        self.name = timeseries.name
        self.annotations = {"nwb_group": nwb_group}
        self.description = try_json_field(timeseries.get('description', ''))
        ...
        metadata_fields = list(timeseries.get('__nwbfields__', []))
